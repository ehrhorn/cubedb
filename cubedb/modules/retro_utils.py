...
def infer_power(edges, dtype=None):
    ...
    try:
        ...
        raise ValueError(
            "Edges do not appear to be power-spaced" " (optimizer did not converge)"
        )
    diff = inv_power_2nd_diff(power, edges)
    if not np.allclose(diff, diff[0], atol=1000 * atol, rtol=10 * rtol):
        raise ValueError(
            "Edges do not appear to be power-spaced"
...
def generate_digitizer(bin_edges, clip=True, handle_under_overflow=True):
    ...
    if power == 1:
        ...
        recip_dx = 1 / dx
        bindescr = "{} bins linearly spaced from {} to {}".format(num_bins, start, stop)
        ...
        bindescr = "{} bins spaced from {} to {} spaced with power of {}".format(
            num_bins, start, stop, power
        )
            ...
            if handle_under_overflow:
                ...
                def digitize(val):
                    ...
                    if val > stop:
                        return overflow_idx
                    idx = int(
                        (val ** recip_power - start_recip_power) * recip_power_width
                    )
                    return min(max(0, idx), num_bins - 1)
        ...
        bindescr = "{} bins logarithmically spaced from {} to {}".format(
            num_bins, start, stop
        )
    ...
    if bindescr is None:
        bindescr = "{} bins unevenly spaced from {} to {}".format(num_bins, start, stop)
    ...
    digitize.__doc__ = """Find bin index for a value.
